...
use troc_core::{DataWriterParamsBuilder, DomainParticipant, DomainParticipantParamsBuilder};
use troc_core::{HistoryQosPolicy, ReliabilityQosPolicy, TopicKind};
...
async fn main() {
    ...
    let mut domain_participant = DomainParticipant::new(0).await;
    ...
    let writer_params = DataWriterParamsBuilder::new()
        .with_name("ExampleWriter")
        .build();
    let mut data_writer = publisher
        .create_datawriter_with_params::<Message>(&topic, &qos, writer_params)
        .await
        .unwrap();
