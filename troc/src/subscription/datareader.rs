...
use bytes::BytesMut;
use protocol::{DdsError, IncommingMessage, OutcommingMessage, Reader, WriterProxy};
use protocol::{
    deserialize_data,
    messages::Message,
...
use crate::{
    ...
    subscription::{
        ...
        reader_adapters::{AckSchedule, AckScheduleReceiverAdapter, ChangeAvailabilityAdapter},
...
pub struct DataReader<T> {
    ...
    change_availability_adapter: ChangeAvailabilityAdapter,
...
impl<T> DataReader<T> {
    ...
    pub async fn read_next_sample_raw(&mut self) -> Result<DataSample<SerializedData>, DdsError> {
        loop {
            ...
            self.change_availability_adapter.wait().await;
        }
    }
...
mod tests {
    ...
    use common::types::{Guid, InlineQos, Locator};
    use protocol::{ReaderBuilder, ReaderConfiguration};
    use rstest::*;
    ...
    use crate::{
        ...
        subscription::{
            datareader::{DataReader, EmissionInfosStorage},
            reader_adapters::{AckScheduleAdapterBuilder, ChangeAvailabilityAdapter},
        },
        wires::WireFactory,
    };
