...
pub async fn three_participants(
    ...
    let alpha_params = DomainParticipantParamsBuilder::new()
        ...
        .build();
    let mut alpha_domain_participant =
        DomainParticipant::new_with_params(domain_id, alpha_params).await;
    ...
    let alpha_reader_params = DataReaderParamsBuilder::new()
        .with_name("AlphaTestReader")
        .enable_listener()
        .build();
    let alpha_reader = alpha_subscriber
        .create_datareader_with_params::<DummyStruct>(&topic, &qos, alpha_reader_params)
        .await
        .unwrap();
    let mut alpha_reader_listener = alpha_reader.get_listener().await.unwrap();

    let alpha_writer_params = DataWriterParamsBuilder::new()
        .with_name("AlphaTestWriter")
        .enable_listener()
        .build();
    let alpha_writer = alpha_publisher
        .create_datawriter_with_params::<DummyStruct>(&topic, &qos, alpha_writer_params)
        .await
        .unwrap();
    ...
    let beta_params = DomainParticipantParamsBuilder::new()
        ...
        .build();
    let mut beta_domain_participant =
        DomainParticipant::new_with_params(domain_id, beta_params).await;
    ...
    let beta_reader_params = DataReaderParamsBuilder::new()
        .with_name("BetaTestReader")
        .enable_listener()
        .build();
    let beta_reader = beta_subscriber
        .create_datareader_with_params::<DummyStruct>(&topic, &qos, beta_reader_params)
        .await
        .unwrap();
    let mut beta_reader_listener = beta_reader.get_listener().await.unwrap();

    let beta_writer_params = DataWriterParamsBuilder::new()
        .with_name("BetaTestWriter")
        .enable_listener()
        .build();
    let beta_writer = beta_publisher
        .create_datawriter_with_params::<DummyStruct>(&topic, &qos, beta_writer_params)
        .await
        .unwrap();
    ...
    let gamma_params = DomainParticipantParamsBuilder::new()
        ...
        .with_config(configuration)
        .build();
    let mut gamma_domain_participant =
        DomainParticipant::new_with_params(domain_id, gamma_params).await;
    ...
    let gamma_reader_params = DataReaderParamsBuilder::new()
        .with_name("GammaTestReader")
        .enable_listener()
        .build();
    let gamma_reader = gamma_subscriber
        .create_datareader_with_params::<DummyStruct>(&topic, &qos, gamma_reader_params)
        .await
        .unwrap();
    let mut gamma_reader_listener = gamma_reader.get_listener().await.unwrap();

    let gamma_writer_params = DataWriterParamsBuilder::new()
        .with_name("GammaTestWriter")
        .enable_listener()
        .build();
    let gamma_writer = gamma_publisher
        .create_datawriter_with_params::<DummyStruct>(&topic, &qos, gamma_writer_params)
        .await
        .unwrap();
