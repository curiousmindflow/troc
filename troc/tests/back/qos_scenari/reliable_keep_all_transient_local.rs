...
#[tokio::test]
async fn one_to_one(
    #[future]#[with("/test/reliable_keep_all_transient_local/one_to_one", ReliabilityQosPolicy::Reliable { max_blocking_time: Timestamp::default() }, HistoryQosPolicy::KeepAll, DurabilityQosPolicy::TransientLocal)] custom_two_participants: (DataReader<DummyStruct>, DataWriter<DummyStruct>),
) {
...
async fn test_many_writers_single_reader(
    #[future] #[with("/test/reliable_keep_all_transient_local/test_many_writers_single_reader", 3, ReliabilityQosPolicy::Reliable { max_blocking_time: Timestamp::default() }, HistoryQosPolicy::KeepAll, DurabilityQosPolicy::TransientLocal)] multiple_writers_single_reader: (DataReader<DummyStruct>, Vec<(DomainParticipant, Publisher, DataWriter<DummyStruct>)>),
) {
...
async fn test_many_readers_single_writer(
    #[future] #[with("/test/reliable_keep_all_transient_local/test_many_readers_single_writer", 3, ReliabilityQosPolicy::Reliable { max_blocking_time: Timestamp::default() }, HistoryQosPolicy::KeepAll, DurabilityQosPolicy::TransientLocal)] multiple_readers_single_writer: (DataWriter<DummyStruct>, Vec<(DomainParticipant, Subscriber, DataReader<DummyStruct>)>),
) {
...
#[tokio::test]
async fn test_mesh(
    #[future] #[with("/test/reliable_keep_all_transient_local/test_mesh", ReliabilityQosPolicy::Reliable { max_blocking_time: Timestamp::default() }, HistoryQosPolicy::KeepAll, DurabilityQosPolicy::TransientLocal)] mesh: (Vec<(i32, i32, DataWriter<DummyStruct>)>, Vec<(i32, i32, DataReader<DummyStruct>)>),
) {
