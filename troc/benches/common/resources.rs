...
use troc_core::{
    DDSType, DataReader, DataWriter, DomainParticipant, HistoryQosPolicy, KeyCalculationError,
    Keyed, Publisher, ReliabilityQosPolicy, Subscriber,
};
use troc_core::{
    DataReaderParamsBuilder, DataWriterParamsBuilder, SerializedData, deserialize_data,
    serialize_data,
};
use troc_core::{ReliabilityKind, TopicKind};
...
impl SimpleDDSBundle {
    pub async fn new(
        ...
        let mut alpha_p = DomainParticipant::new(0).await;
        ...
        let alpha_reader_params = DataReaderParamsBuilder::new().enable_listener().build();
        let alpha_reader = alpha_sub
            .create_datareader_with_params(&alpha_topic, &alpha_qos, alpha_reader_params)
            .await
            .unwrap();
        let mut alpha_reader_listener = alpha_reader.get_listener().await.unwrap();

        let mut beta_p = DomainParticipant::new(0).await;
        let beta_qos = beta_p
            .create_qos_builder()
            .reliability(reliability)
            .history(history_qos)
        ...
        let beta_writer_params = DataWriterParamsBuilder::new().enable_listener().build();
        let beta_writer = beta_pub
            .create_datawriter_with_params(&beta_topic, &beta_qos, beta_writer_params)
            .await
            .unwrap();
...
impl OneWriterManyReaderDDSBundle {
    pub async fn new(
        ...
        let mut gamma_p = DomainParticipant::new(0).await;
        ...
        let gamma_reader_params = DataReaderParamsBuilder::new().enable_listener().build();
        let gamma_reader = gamma_sub
            .create_datareader_with_params(&gamma_topic, &gamma_qos, gamma_reader_params)
            .await
            .unwrap();
...
pub async fn udp_exchange(
    ...
    let payload = serialize_data(&msg).unwrap();
    ...
    let msg = SerializedData::new(&payload_buf);
    deserialize_data(&msg).unwrap()
}
