...
impl WriterBuilder {
    pub fn new(guid: Guid, qos: InlineQos, config: WriterConfiguration) -> Self {
        Self {
            guid,
            qos,
            is_reliable: ReliabilityKind::BestEffort,
            config,
        }
    }

    pub fn reliable(mut self, reliable: ReliabilityKind) -> Self {
...
impl Writer {
    ...
    pub fn extract_proxy(&self) -> WriterProxy {
        WriterProxy {
            remote_writer_guid: self.get_guid(),
            // FIXME: should not be hardcoded, at least not here
            data_max_size_serialized: 64 * 1024,
            ..Default::default()
        }
    }
}
...
mod test {
    use crate::{
        ...
        types::{
            ChangeKind, ContentNature, Count, EntityId, Guid, InlineQos, InstanceHandle,
            LocatorList, ReliabilityKind, SequenceNumber, SequenceNumberSet, SerializedData,
            Timestamp,
        },
    };
    ...
    use crate::{
        CacheChange, CacheChangeInfos, Effects, ReaderProxy,
        common::{
    ...
    fn setup_writer(
        ...
        let mut writer = WriterBuilder::new(Guid::default(), qos, conf)
            .reliable(reliable)
            .build();
