...
#[cfg(test)]
mod parameter_list_tests {
    use std::io::Cursor;

    use binrw::{BinRead, BinWrite, Endian};

    use crate::types::{parameter::Parameter, parameter_id::ParameterId};

    use super::ParameterList;

    static SERIALIZED_PARAM_LIST_NOPAD: &[u8] = &[
        0x00, 0x02, 0x00, 0x04, 0x01, 0x02, 0x03, 0x04, 0x00, 0x03, 0x00, 0x02, 0x01, 0x02, 0x00,
        0x01, 0x00, 0x00,
    ];

    static SERIALIZED_PARAM_LIST_WITHPAD: &[u8] = &[
        0x00, 0x02, 0x00, 0x03, 0x01, 0x02, 0x03, 0x00, 0x00, 0x00, 0x03, 0xFF, 0xFF, 0xFF, 0x00,
        0x03, 0x00, 0x02, 0x01, 0x02, 0x00, 0x01, 0x00, 0x00,
    ];

    // FIXME: to test the correct layout of ParameterList, the PADDING / SENTINEL special parameter integration must be done in the ParameterList builder, instead of the Serialize/Deserialize custom methods
    #[test]
    fn serialize_nopad_test() {
        let expected = SERIALIZED_PARAM_LIST_NOPAD;
        ...
        param_list
            .write_options(&mut actual, Endian::Big, ())
            .unwrap();
    ...
    #[test]
    fn serialize_withpad_test() {
        let expected = SERIALIZED_PARAM_LIST_WITHPAD;
    ...
    fn deserialize_nopad_test() {
        ...
        let mut bytes = Cursor::new(SERIALIZED_PARAM_LIST_NOPAD);
        let actual: ParameterList =
            ParameterList::read_options(&mut bytes, Endian::Big, ()).unwrap();

        assert_eq!(expected, actual);
    }

    #[test]
    fn deserialize_withpad_test() {
        let expected = ParameterList {
            parameters: vec![
                Parameter {
                    parameter_id: ParameterId(2),
        ...
        let mut bytes = Cursor::new(SERIALIZED_PARAM_LIST_WITHPAD);
        let actual: ParameterList =
            ParameterList::read_options(&mut bytes, Endian::Big, ()).unwrap();
